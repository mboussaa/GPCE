...
However, based on existing benchmarks between technical environments~\cite{}, in comparing the behavior of the generated code of a large data set of programs, our feeling is that we could detect inefficient code generators. The kind of error that we track is the lack of use of a specific function, a specific type or a specific language feature in a targeted environment that can speed or reduce the memory consumption of the resulting program. The main difficulties is the fact that, for testing the non functional properties of a code generator, we cannot just observe the execution of the code generator but we have to observe and compare the execution of the generated program. Even if there is no exact oracle to detect an inconsistencies, we could benefit from the family of code generator to compare the results of two program that runs atop two different technical stacks. 
