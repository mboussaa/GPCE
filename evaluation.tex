...
In fact, throughout the inspection of generated source code, we remark the intensive use of \textit{"arrays"} in most of the functions under test. In most languages, arrays are fixed-sized, but this is not the case in PHP since they are allocated dynamically. The dynamic allocation of arrays  leads to a slower write time because the memory locations needed to hold the new data is not already allocated. Thus, slow writing speed damages the performance of PHP code. This observation clearly confirm our findings. The solution for this problem may be to use another type of object from the Standard PHP Library. As an alternative, \textit{"SplFixedArray"} pre-allocates the necessary memory thereby solving the issue of slower write times.
